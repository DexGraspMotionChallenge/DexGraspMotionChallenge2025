...
def test_env(args, task, env, model, bc_model_name,obj_id, global_feat=None,use_gt=False, use_part_gt=False):
    ...
    rollout_reward_sum = torch.zeros(task.num_envs, dtype=torch.float, device=args.rl_device)
    rollout_steps = 0
    ...
    ever_success = torch.zeros(task.num_envs, dtype=torch.bool, device=args.rl_device)
    peak_success_mask = torch.zeros(task.num_envs, dtype=torch.bool, device=args.rl_device)
    ...
    while len(reward_sum) < maxlen:
        ...
        current_success_mask = task.successes > 0
        current_success_count = int(current_success_mask.sum().item())
        if current_success_count > peak_success_count:
            peak_success_count = current_success_count
            peak_success_mask = current_success_mask.clone()
        cur_success_done_sum[new_ids] = cur_success[new_ids]
        if task.cfg['env'].get('evaluation_step_logging', False) and task.successes.sum()>0:
            print('name={}, iter={}, N_seq={}, success_num:{}'.format(obj_id, i ,task.num_envs, task.successes.sum()))
            a=1
        elif task.cfg['env'].get('evaluation_step_logging', False):
        ...
        if (i>15 and model.__class__.__name__ == 'LitDP3Model') or (i>120 and model.__class__.__name__ == 'LitBCModel'):
            ...
            official_peak_rate = peak_success_count / task.num_envs
            ever_success_count = int(ever_success.sum().item())
            ever_success_rate = ever_success_count / task.num_envs
            if args.success_metric == 'ever':
                succ_sum = ever_success_count
                succ_rate = ever_success_rate
                selected_success_mask = ever_success
            else:
                succ_sum = peak_success_count
                succ_rate = official_peak_rate
                selected_success_mask = peak_success_mask
            mean_rollout_reward = rollout_reward_sum.mean().item()
            success_env_indices = selected_success_mask.nonzero(
                as_tuple=False).flatten().cpu().tolist()

            print('Official peak success {}::> {:.2f}%'.format(
                obj_id, official_peak_rate * 100))
            print('Diagnostic ever-success {}::> {:.2f}%'.format(
                obj_id, ever_success_rate * 100))
            print('Selected metric {}::> {} ({:.2f}%)'.format(
                obj_id, args.success_metric, succ_rate * 100))
            print(f'Mean Rollout Reward {obj_id}::> {mean_rollout_reward:.4f} ({rollout_steps} steps)')

            result_desc = (
                'name={}, N_seq={}, success_metric:{},success_num:{},success_rate:{},'
                'official_peak_success_rate:{},diagnostic_ever_success_rate:{},'
                'mean_rollout_reward:{},rollout_steps:{}'
            ).format(obj_id, task.num_envs, args.success_metric, succ_sum, succ_rate,
                     official_peak_rate, ever_success_rate,
                     mean_rollout_reward, rollout_steps)
            metrics = {
                'success_metric': args.success_metric,
                'success_count': succ_sum,
                'success_rate': succ_rate,
                'official_peak_success_count': peak_success_count,
                'official_peak_success_rate': official_peak_rate,
                'diagnostic_ever_success_count': ever_success_count,
                'diagnostic_ever_success_rate': ever_success_rate,
                'mean_rollout_reward': mean_rollout_reward,
                'rollout_steps': rollout_steps,
                'success_env_indices': success_env_indices,
            }
            print(result_desc)
            break
