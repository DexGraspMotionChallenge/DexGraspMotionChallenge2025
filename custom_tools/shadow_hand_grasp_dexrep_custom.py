...
class CustomShadowHandGraspDexRepIjrr(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless,
        ...
        # Challenge-compatible evaluation keeps the official zero reward.
        # Dense reward is available only when a custom diagnostic run asks for it.
        self.diagnostic_dense_reward = bool(
            self.cfg["env"].get("diagnostic_dense_reward", False))
        ...
        if cfg['env']['env_mode']=='rl_learning':
            ...
            if npy_list is not None:
                object_codes = [data.get('obj_code') for data in npy_list]
                if any(code is None for code in object_codes):
                    raise KeyError('Each in-memory trajectory dictionary needs an obj_code field.')
        ...
        available_num_envs = self.obj_trajs_info['grasp_seqs'].shape[0]
        trajectory_indices = self.cfg['env'].get('trajectory_indices', [])
        if trajectory_indices:
            invalid_indices = [index for index in trajectory_indices
                               if index < 0 or index >= available_num_envs]
            if invalid_indices:
                raise IndexError(
                    'Trajectory indices {} are outside [0, {}).'.format(
                        invalid_indices, available_num_envs)
                )
            for key, value in self.obj_trajs_info.items():
                if isinstance(value, np.ndarray) and value.shape[0] == available_num_envs:
                    self.obj_trajs_info[key] = value[trajectory_indices]
            self.object_idxs = [self.object_idxs[index] for index in trajectory_indices]

        # A trajectory file normally contains 40 rollouts, so it used to
        # override the --num_envs command-line option below.  Respect a
        # smaller requested value for low-memory smoke tests while keeping
        # the default challenge/evaluation behavior unchanged.
        requested_num_envs = self.cfg['env'].get('numEnvs', 0)
        available_num_envs = self.obj_trajs_info['grasp_seqs'].shape[0]
        if 0 < requested_num_envs < available_num_envs:
            for key, value in self.obj_trajs_info.items():
                if isinstance(value, np.ndarray) and value.shape[0] == available_num_envs:
                    self.obj_trajs_info[key] = value[:requested_num_envs]
            self.object_idxs = self.object_idxs[:requested_num_envs]
    ...
    def _setup_capture_camera(self):
        ...
        self.capture_first_frame = None
        self.capture_last_frame = None
        self.capture_success_frame = None
        self.capture_frame_count = 0
        self.capture_dir = self.cfg['env'].get('capture_dir', '')
        if not self.capture_dir:
            return

        capture_env = int(self.cfg['env'].get('capture_env', 0))
        if capture_env < 0 or capture_env >= self.num_envs:
            raise IndexError('Capture environment {} is outside [0, {}).'.format(
                capture_env, self.num_envs))

        import cv2
        os.makedirs(self.capture_dir, exist_ok=True)
        self.capture_env = capture_env
        self.capture_width = int(self.cfg['env'].get('capture_width', 640))
        self.capture_height = int(self.cfg['env'].get('capture_height', 480))
        self.capture_stride = max(1, int(self.cfg['env'].get('capture_stride', 1)))
        self.capture_video_path = os.path.join(
            self.capture_dir, 'env{:03d}.mp4'.format(capture_env))
        self.capture_first_path = os.path.join(
            self.capture_dir, 'env{:03d}_first.png'.format(capture_env))
        self.capture_final_path = os.path.join(
            self.capture_dir, 'env{:03d}_final.png'.format(capture_env))
        self.capture_success_path = os.path.join(
            self.capture_dir, 'env{:03d}_success.png'.format(capture_env))
        existing_outputs = [path for path in (
            self.capture_video_path, self.capture_first_path, self.capture_final_path,
            self.capture_success_path)
                            if os.path.exists(path)]
        if existing_outputs:
            raise FileExistsError(
                'Capture outputs already exist: {}. Choose a new --capture_dir.'.format(
                    ', '.join(existing_outputs)))

        camera_properties = gymapi.CameraProperties()
        camera_properties.width = self.capture_width
        camera_properties.height = self.capture_height
        ...
        fps = max(1.0, 20.0 / self.capture_stride)
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        self.capture_writer = cv2.VideoWriter(
            self.capture_video_path, fourcc, fps,
            (self.capture_width, self.capture_height))
        if not self.capture_writer.isOpened():
            raise RuntimeError('Could not open video writer: {}'.format(self.capture_video_path))

    def capture_frame(self, policy_step):
        if self.capture_camera is None or policy_step % self.capture_stride != 0:
            return
        ...
        rgba = np.asarray(image, dtype=np.uint8).reshape(
            self.capture_height, self.capture_width, 4)
        bgr = cv2.cvtColor(rgba, cv2.COLOR_RGBA2BGR)
        if self.capture_first_frame is None:
            self.capture_first_frame = bgr.copy()
            cv2.imwrite(self.capture_first_path, self.capture_first_frame)
        if (self.capture_success_frame is None
                and self.successes[self.capture_env].item() > 0):
            self.capture_success_frame = bgr.copy()
            cv2.imwrite(self.capture_success_path, self.capture_success_frame)
        self.capture_last_frame = bgr.copy()
        self.capture_writer.write(bgr)
    ...
    def _finalize_capture(self):
        if self.capture_writer is not None:
            ...
            self.capture_writer = None
            if self.capture_last_frame is not None:
                cv2.imwrite(self.capture_final_path, self.capture_last_frame)
            print('Saved {} render frames to {}'.format(
                self.capture_frame_count, self.capture_dir))
...
def compute_hand_reward(
        ...
        goal_cond: bool, diagnostic_dense_reward: bool
):
    ...
    right_hand_dist = torch.where(
        right_hand_dist >= 0.5, 0.5 + 0 * right_hand_dist, right_hand_dist)

    right_hand_finger_dist = (
        torch.norm(object_handle_pos - right_hand_ff_pos, p=2, dim=-1)
        + torch.norm(object_handle_pos - right_hand_mf_pos, p=2, dim=-1)
        + torch.norm(object_handle_pos - right_hand_rf_pos, p=2, dim=-1)
        + torch.norm(object_handle_pos - right_hand_lf_pos, p=2, dim=-1)
        + torch.norm(object_handle_pos - right_hand_th_pos, p=2, dim=-1)
    )
    right_hand_finger_dist = torch.where(
        right_hand_finger_dist >= 3.0,
        3.0 + 0 * right_hand_finger_dist,
        right_hand_finger_dist,
    )

    close_to_object = (
        (right_hand_finger_dist <= 0.6).int()
        + (right_hand_dist <= 0.12).int()
    ) == 2
    goal_hand_rew = torch.where(
        close_to_object, 0.9 - 2 * goal_dist, torch.zeros_like(goal_dist))
    hand_up = torch.where(
        (object_pos[:, 2] >= 0.630) & close_to_object,
        0.1 + 0.1 * actions[:, 2],
        torch.zeros_like(goal_dist),
    )
    hand_up = torch.where(
        (object_pos[:, 2] >= 0.80) & close_to_object,
        0.2 - 0 * goal_hand_dist,
        hand_up,
    )
    bonus = torch.where(
        close_to_object & (goal_dist <= 0.05),
        1.0 / (1 + 10 * goal_dist),
        torch.zeros_like(goal_dist),
    )

    if diagnostic_dense_reward:
        # Optional custom diagnostic signal. It does not change actions,
        # physics, or success conditions, and is never called "official reward".
        reward = (
        ...
        # Exact behavior of the original challenge task.
        reward = torch.zeros_like(goal_dist)
