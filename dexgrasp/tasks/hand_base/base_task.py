...
class BaseTask():
    ...
    def __init__(self, cfg, enable_camera_sensors=False, is_meta=False, task_num=0):
        ...
        # allocate buffers
        if self.obs_type == 'obs_pcds' or self.obs_type == 'obs_h2o':
    ...
    def step(self, actions, id):
        ...
        if id == -1:
            self.init_obj_pos = self.root_state_tensor.view(self.num_envs, -1, 13)[:, self.objects[0], :3].clone()
            # self.init_obj_quat = self.root_state_tensor.view(self.num_envs, -1, 13)[:, self.objects[0], 3:7].clone()
